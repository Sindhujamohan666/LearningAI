...
- Chunk documents, embed with a sentence-transformer model, store in ChromaDB
...
- CLI interface:
  ```bash
  python qabuddy.py --project ./data --framework playwright --llm ollama
  ```
- Optional Streamlit web UI for interactive use
...
| Embeddings | sentence-transformers / Ollama | Offline capable |
| Test Execution | Playwright (primary), Selenium (fallback) | Modern, cross-browser |
| Reporting | Jinja2 → HTML, WeasyPrint → PDF | Template-based |
| UI (optional) | Streamlit | Fast prototyping |
...
├── Prompt/image.png                 # Prompt reference image
...
│   │   ├── document_analyzer.py     # Phase 2: RAG pipeline
│   │   ├── test_generator.py        # Phase 3: Test case generation
│   │   ├── test_executor.py         # Phase 4: Test execution
│   │   ├── jira_agent.py            # Phase 5: JIRA integration
│   │   ├── cicd_agent.py            # Phase 6: CI/CD templates
│   │   └── reporter.py              # Phase 7: Reporting & RTM
│   ├── core/
...
│   │   └── rtm.html.j2              # RTM template
│   └── cli.py                       # CLI entry point
├── tests/                           # Tests for QABuddyAI itself
├── requirements.txt
├── .env.example
└── README.md
```
